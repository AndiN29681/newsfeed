...
export default function SearchPage() {
  ...
  const [rssProgress, setRssProgress] = useState<{ loaded: number; total: number }>({
    loaded: 0,
    total: germanFeeds.length,
  })
  ...
  const searchNews = async (searchTerm: string, page = 1) => {
    ...
    try {
      ...
      const rssResults = await new Promise<{ title: string; content: string; link: string }[]>(
        (resolve) => {
      ...
      // Create clean versions for summary
            ...
            <div className="flex items-center">
              ...
              <div className="w-32 h-2 bg-blue-200 dark:bg-blue-800 rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${(rssProgress.loaded / rssProgress.total) * 100}%`,
                  }}
                ></div>
              </div>
            </div>
